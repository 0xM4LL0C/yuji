...
YUJI_VERSION_PATCH ?= 1
YUJI_VERSION_PRE ?=
YUJI_VERSION_BUILD ?=

YUJI_VERSION_BASE := $(YUJI_VERSION_MAJOR).$(YUJI_VERSION_MINOR).$(YUJI_VERSION_PATCH)

ifneq ($(strip $(YUJI_VERSION_PRE)),)
	YUJI_VERSION_BASE := $(YUJI_VERSION_BASE)-$(YUJI_VERSION_PRE)
endif

ifneq ($(strip $(YUJI_VERSION_BUILD)),)
	YUJI_VERSION_BASE := $(YUJI_VERSION_BASE)+$(YUJI_VERSION_BUILD)
endif

YUJI_VERSION_STRING := $(YUJI_VERSION_BASE)
...
CFLAGS  ?= -Iinclude -std=gnu99 -Wall -Wextra -Wshadow -Wconversion \
	...
	-DYUJI_VERSION_PRE=$(YUJI_VERSION_PRE) \
	-DYUJI_VERSION_BUILD=$(YUJI_VERSION_BUILD) \
