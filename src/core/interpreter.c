  ...
  switch (node->type) {
    ...
    case YUJI_AST_BLOCK: {
      ...
      YUJI_DYN_ARRAY_ITER(node->value.block->exprs, YujiASTNode, expr, {
        yuji_interpreter_eval(interpreter, expr);
      })
    ...
    case YUJI_AST_CALL: {
      ...
      if (!fn_val || fn_val->type != VT_FUNCTION) {
    ...
    case YUJI_AST_WHILE: {
      ...
      while (yuji_value_to_bool(yuji_interpreter_eval(interpreter,
                                node->value.while_stmt->condition))) {
        YUJI_DYN_ARRAY_ITER(node->value.while_stmt->body->exprs, YujiASTNode, body, {
          yuji_value_free(result);
          result = yuji_interpreter_eval(interpreter, body);
    ...
    case YUJI_AST_IF: {
      YUJI_DYN_ARRAY_ITER(node->value.if_stmt->branches, YujiASTIfBranch, branch, {
        if (yuji_value_to_bool(yuji_interpreter_eval(interpreter, branch->condition))) {
      ...
      if (node->value.if_stmt->else_body) {
        YUJI_DYN_ARRAY_ITER(node->value.if_stmt->else_body->exprs, YujiASTNode, expr, {
          return yuji_interpreter_eval(interpreter, expr);
        })
      }
